	...
	public class Texture : IShaderView, IDisposable
	{
		...
		public void SaveFile(string fileName)
		{
			if (buffer_.Resource is Texture2D) {
				Texture2D.SaveTextureToFile(Renderer.D3dImmediateContext, buffer_.Resource, ImageFileFormat.Dds, fileName);
			} else if (buffer_.Resource is Texture3D) {
				Texture3D.SaveTextureToFile(Renderer.D3dImmediateContext, buffer_.Resource, ImageFileFormat.Dds, fileName);
			}
		}
	}
}
